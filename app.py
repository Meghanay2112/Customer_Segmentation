...
.hero-sub { color: #94a3b8; font-size: 1rem; letter-spacing: 0.02em; }
...
.metric-value { font-family: 'Space Mono', monospace; font-size: 2rem; font-weight: 700; color: #38bdf8; }
.metric-delta { font-size: 0.75rem; color: #4ade80; margin-top: 0.2rem; }
...
.stTabs [data-baseweb="tab-list"] { background: #0e1120; border-radius: 10px; gap: 4px; padding: 4px; }
.stTabs [data-baseweb="tab"] { color: #94a3b8 !important; font-family: 'DM Sans', sans-serif; font-weight: 500; border-radius: 7px; }
.stTabs [aria-selected="true"] { background: linear-gradient(135deg, #1e3a5f, #162547) !important; color: #38bdf8 !important; }

.stButton > button {
    background: linear-gradient(135deg, #1d4ed8 0%, #7c3aed 100%);
    color: white; border: none; border-radius: 8px;
    font-family: 'DM Sans', sans-serif; font-weight: 600;
    padding: 0.6rem 1.4rem; transition: opacity 0.2s, transform 0.15s; width: 100%;
}
...
def generate_dataset(n_samples: int = 5000, random_state: int = 42) -> pd.DataFrame:
    ...
    seg[(annual_income > 90000) & (spending_score > 60) & (frequency > 10)] = 0
    seg[(annual_income > 70000) & (spending_score > 45) & (loyalty_years > 3) & (seg == 0)] = 1
    seg[(recency < 15) & (frequency > 6) & (monetary > 2000) & (seg == 0)] = 2
    seg[(recency > 200) & (seg == 0)] = 3
    seg[(recency > 300) & (monetary < 500) & (seg == 0)] = 4
    mask = seg == 0
...
# ─── Model Builder ────────────────────────────────────────────────────────────
def build_model(units: list, dropout: float, epochs: int) -> MLPClassifier:
...
def plot_confusion_matrix(cm, class_names):
    ...
    im = ax.imshow(cm_norm, cmap=plt.cm.Blues, vmin=0, vmax=1, aspect="auto")
    for i in range(len(class_names)):
        for j in range(len(class_names)):
            color = "white" if cm_norm[i, j] < 0.55 else "#0d1117"
...
def plot_loss_curve(loss_curve):
    ...
    for spine in ax.spines.values():
        spine.set_edgecolor("#2d3a5e")
    ax.plot(range(1, len(loss_curve) + 1), loss_curve, color="#38bdf8", lw=2, label="Train Loss")
    ax.set_title("Training Loss Curve", color="#e2e8f0", fontfamily="monospace", fontsize=10)
    ax.set_xlabel("Iteration", color="#94a3b8", fontsize=8)
    ax.set_ylabel("Loss", color="#94a3b8", fontsize=8)
    ax.tick_params(colors="#94a3b8")
    ax.legend(framealpha=0, labelcolor="#94a3b8", fontsize=8)
    plt.tight_layout()
    return fig


def plot_feature_importance(model, feature_names, X_sample, y_sample):
    baseline_acc = accuracy_score(y_sample, model.predict(X_sample))
    importance = []
    X_perm = X_sample.copy()
    for i in range(X_perm.shape[1]):
        orig = X_perm[:, i].copy()
        np.random.shuffle(X_perm[:, i])
        perm_acc = accuracy_score(y_sample, model.predict(X_perm))
        importance.append(max(0.0, baseline_acc - perm_acc))
        X_perm[:, i] = orig
...
def plot_pca_clusters(X_scaled, y, segment_names, segment_colors):
    ...
    ax.legend(framealpha=0.1, labelcolor="#e2e8f0", fontsize=7.5,
...
with st.sidebar:
    ...
    <b style='color:#38bdf8'>Model</b>: sklearn MLPClassifier<br>
    <b style='color:#38bdf8'>Optimizer</b>: Adam + Adaptive LR<br>
    <b style='color:#38bdf8'>Regularization</b>: L2 + Early Stopping<br>
...
<div class='hero-banner'>
  ...
  <div class='hero-sub'>
    Multi-layer neural network &nbsp;·&nbsp; RFM + behavioural features &nbsp;·&nbsp;
...
if train_btn:
    st.session_state.trained = False
    with st.spinner("🔄 Training neural network… this may take a few seconds"):
        model = build_model(units, dropout, epochs)
    ...
    st.session_state.update({
        ...
        "report": report, "loss_curve": loss_curve,
...
if st.session_state.trained:
    ...
    with tab1:
        ...
        with col_right:
            ...
            fig_pca = plot_pca_clusters(X_scaled_full[:2000], y_full[:2000], SEGMENT_NAMES, SEGMENT_COLORS)
    ...
    with tab2:
        ...
        fig_loss = plot_loss_curve(loss_curve)
        st.pyplot(fig_loss, use_container_width=True)
        plt.close(fig_loss)
        st.info(f"Model converged in **{st.session_state.n_iter}** iterations "
                f"(early stopping patience = 12).")

    # ── Tab 3 ────────────────────────────────────────────────────────────────
    with tab3:
        st.markdown("<div class='section-header'>Permutation Feature Importance</div>", unsafe_allow_html=True)
        with st.spinner("Computing feature importance…"):
            fig_fi = plot_feature_importance(model, FEATURES, X_test[:500], y_test[:500])
        st.pyplot(fig_fi, use_container_width=True)
        plt.close(fig_fi)
        st.info("Feature importance is computed by permuting each feature and measuring "
                "the drop in accuracy across all classes.")

    # ── Tab 4 ────────────────────────────────────────────────────────────────
    with tab4:
        st.markdown("<div class='section-header'>Predict Segment for a New Customer</div>", unsafe_allow_html=True)
    ...
    with col2:
        st.markdown("<div class='section-header'>Segment Legend</div>", unsafe_allow_html=True)
